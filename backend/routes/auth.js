...
// Strict rate limiter for registration — prevents automated sign-ups
...
const csrfOriginGuard = (req, res, next) => {
    ...
    // Cross-origin form submissions that omit Origin are a CSRF vector
    // because the httpOnly refresh cookie is sent automatically.
    if (!origin) {
        if (process.env.NODE_ENV === 'production') {
            return res.status(403).json({ error: 'CSRF check failed: Origin header required' });
        }
...
router.post('/register', registerLimiter, validate(registerSchema), asyncHandler(async (req, res) => {
    const { clinicName, email, password, phone, inviteCode } = req.body;

    // Invite-code gate. Two modes:
    //  - REGISTRATION_INVITE_CODE unset  → registration is fully disabled
    //    (high-touch onboarding; use POST /api/leads/demo instead)
    //  - REGISTRATION_INVITE_CODE set    → registration is enabled only when
    //    the request supplies the matching invite code (passed by /demo-request
    //    flow once the platform owner agrees to provision a clinic)
    const systemInviteCode = process.env.REGISTRATION_INVITE_CODE;
    if (!systemInviteCode) {
        throw new AppError(
            'REGISTRATION_DISABLED',
            'Self-serve registration is disabled. Please request a demo at /demo-request and our team will set up your clinic manually.',
            403
        );
    }
    if (inviteCode !== systemInviteCode) {
        throw new AppError('FORBIDDEN', 'Invalid invite code. Registration is currently restricted.', 403);
    }
    ...
    try {
        ...
        const registrationData = await prisma.$transaction(async (tx) => {
            // High-touch onboarding: trialEndsAt is intentionally NOT set here.
            // It will be set when the platform owner marks the clinic as
            // "go-live" via POST /api/clinic/onboarding-complete. Until then
            // the clinic can use the product without a ticking trial clock.
            const clinic = await tx.clinic.create({
                data: {
                    ...
                    webhookSecret: encrypt(crypto.randomBytes(32).toString('hex')), // Generate unique secret
                    plan: 'trial',
                    planStatus: 'trialing',
                    trialEndsAt: null,
                }
            });
            const plainSecret = decrypt(clinic.webhookSecret);
        ...
        const { user: result, plainSecret } = registrationData;
...
router.post('/forgot-password', passwordResetLimiter, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        logger.warn('Password reset email failed', { email: user.email, err });
    }
    ...
    if (!emailSent && user.clinic?.webhookUrl) {
        ...
        ).catch((err) => logger.error('Password reset webhook fallback failed', { err }));
    }
...
router.post('/google', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const googleAuthData = await prisma.$transaction(async (tx) => {
            ...
            if (!c) {
                c = await tx.clinic.create({
                    data: {
                        ...
                        services: '[]',
                        policies: '{}',
                        webhookSecret: encrypt(crypto.randomBytes(32).toString('hex')),
                    }
                });
                plainSecret = decrypt(c.webhookSecret);
            ...
            return { clinic: c, user: u, isAdmin: u.role === 'ADMIN' || u.role === 'OWNER', plainSecret };
        });

        const { clinic, user, isAdmin, plainSecret } = googleAuthData;
        const accessToken = generateAccessToken({
            userId: user.id,
            clinicId: user.clinicId,
            role: user.role,
        ...
        res.json({
            ...
            clinic: {
                ...
                webhookSecret: plainSecret || null
            }
        });
    } catch (error) {
