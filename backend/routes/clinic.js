...
router.get('/', asyncHandler(async (req, res) => {
    ...
    res.json({ ...data, role: req.user.role, userId: req.user.userId });
}));
...
router.post('/webhooks/test', requireOwner, asyncHandler(async (req, res) => {
    ...
    // Prevent SSRF (Server-Side Request Forgery) by blocking private and internal network ranges
    try {
        const parsed = new URL(url);
        if (!['http:', 'https:'].includes(parsed.protocol)) {
            throw new AppError('VALIDATION_ERROR', 'Invalid URL protocol. Only HTTP and HTTPS are allowed.', 400);
        }
        ...
        const isPrivateIp = /^10\./.test(hostname) ||
            ...
            /^\[::1\]/.test(hostname);
        if (isPrivateIp) {
            throw new AppError('FORBIDDEN', 'Access to private or local network ranges is blocked.', 403);
        }
    } catch (err) {
        if (err instanceof AppError) throw err;
        throw new AppError('VALIDATION_ERROR', 'Malformed URL', 400);
    }
...
router.post('/upgrade-plan', requireOwner, asyncHandler(async (req, res) => {
    ...
    const clinic = await prisma.clinic.findUnique({ where: { id: req.clinicId }, select: { plan: true } });
    const currentKey = getPlanKeyByClinic(clinic);
    const error = validateUpgrade(currentKey, plan);
