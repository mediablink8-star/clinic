...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (!passwordRegex.test(trimmedPassword)) {
        throw new AppError('VALIDATION_ERROR', 'Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, and one number.', 400);
    }

    const allowedRoles = ['OWNER', 'RECEPTIONIST', 'ASSISTANT', 'DOCTOR'];
    if (!allowedRoles.includes(role)) {
        throw new AppError('VALIDATION_ERROR', 'Μη έγκυρος ρόλος.', 400);
    }

    if (role === 'OWNER' && req.user.role !== 'OWNER') {
        throw new AppError('FORBIDDEN', 'Μόνο ο Ιδιοκτήτης (OWNER) μπορεί να δημιουργήσει άλλον Ιδιοκτήτη.', 403);
    }
...
router.put('/:id', asyncHandler(async (req, res) => {
    ...
    if (role === 'OWNER' && req.user.role !== 'OWNER') {
        throw new AppError('FORBIDDEN', 'Μόνο ο Ιδιοκτήτης (OWNER) μπορεί να προαγάγει κάποιον σε Ιδιοκτήτη.', 403);
    }
    ...
    if (target.role === 'OWNER' && req.user.role !== 'OWNER') {
        throw new AppError('FORBIDDEN', 'Μόνο ο Ιδιοκτήτης (OWNER) μπορεί να τροποποιήσει άλλον Ιδιοκτήτη.', 403);
    }
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    if (target.role === 'OWNER' && req.user.role !== 'OWNER') {
        throw new AppError('FORBIDDEN', 'Μόνο ο Ιδιοκτήτης (OWNER) μπορεί να διαγράψει έναν Ιδιοκτήτη.', 403);
    }
