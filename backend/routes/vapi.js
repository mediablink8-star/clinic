...
function vapiAuth(req, res, next) {
    ...
    if (!vapiSecret) {
        ...
        logger.warn('No VAPI_WEBHOOK_SECRET set — running unauthenticated in dev');
        return next();
    }
...
router.post('/webhook', vapiAuth, asyncHandler(async (req, res) => {
    ...
    // Process asynchronously with retry via setImmediate + catch
    setImmediate(async () => {
        const MAX_RETRIES = 3;
    ...
    if (status === 'ended' || status === 'completed') {
        ...
        if (!wasBooked) {
            ...
            if (clinic) {
                ...
                logger.info('Vapi call ended without booking, triggering SMS fallback', { callId: call_id, phoneTail: mc.fromNumber.slice(-4) });
