...
async function parseCommand(command, context = {}) {
    ...
    // SECURITY: Sanitize input to reduce prompt injection surface
    const INJECTION_PATTERNS = [
        /ignore previous instructions?/gi,
        /ignore (all|your) (previous|prior|above) (instructions?|rules?|prompts?)/gi,
        /forget (everything|your instructions|your rules)/gi,
        /system prompt/gi,
        /you are now/gi,
        /new instructions?/gi,
        /override (your|the) (instructions?|rules?|constraints?)/gi,
        /jailbreak/gi,
        /bypass/gi,
    ];
    ...
    try {
        ...
        // SECURITY: Validate output — only allow known actions
        const ALLOWED_ACTIONS = [
            'send_sms', 'call_patient', 'book_appointment',
            'cancel_appointment', 'list_today_appointments', 'list_missed_calls'
        ];
        if (!ALLOWED_ACTIONS.includes(parsed.action)) {
            logger.warn('AI returned disallowed action', { action: parsed.action, command: sanitized });
            return { action: 'unknown', parameters: {}, confidence: 0.0, error: 'Action not allowed' };
        }

        // SECURITY: Sanitize string parameters — strip control chars, cap length
        ...
        logger.error('AI Command parse failed', { err, command });
