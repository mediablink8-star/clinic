...
async function listAppointments(clinicId, doctorId = null, page = 1, limit = 50, deleted = false, user = null, dateFrom = null, dateTo = null) {
    ...
    // Server-side date range filtering — avoids fetching entire history
    if (dateFrom || dateTo) {
        whereClause.startTime = {};
        if (dateFrom) whereClause.startTime.gte = new Date(dateFrom);
        if (dateTo) whereClause.startTime.lte = new Date(dateTo);
    }
    const skip = (page - 1) * limit;
    const [data, total] = await Promise.all([
        ...
        if (clinic) {
            ...
            createCalendarEvent({ clinic, appointment, patient })
                .then(async eventId => {
                    ...
                    // The appointment may have been deleted while the async
                    // calendar insert was in flight. Never resurrect a deleted
                    // appointment as a live calendar event.
