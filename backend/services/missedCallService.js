...
async function handleMissedCall({ phone, clinicId, callSid, bypassCooldown = false, isSilent = false }) {
    ...
    if (clinic.voiceEnabled) {
        logger.info('Voice recovery enabled, attempting AI call', { clinicId, phoneTail: normalizedPhone.slice(-4) });
        ...
        try {
            ...
            logger.warn('Voice Patient lookup failed', { error: err.message });
        }
        ...
        if (callResult.success) {
            logger.info('Voice Outbound call triggered', { phoneTail: normalizedPhone.slice(-4), callId: callResult.callId });
        ...
        logger.warn('Voice Call failed — falling back to SMS', { reason: callResult.reason });
        await prisma.missedCall.update({
            where: { id: missedCall.id },
            data: { smsError: `voice_failed: ${callResult.reason}` }
        });
    } else {
        logger.info('Voice Skipped', { voiceEnabled: clinic.voiceEnabled });
    }
