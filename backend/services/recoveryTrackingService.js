...
async function backfillRecoveryCases({ days = 30, limit = 100 } = {}) {
    ...
    const missedCalls = await prisma.missedCall.findMany({
        where: {
            ...
            // Only backfill those that don't have a case yet
    ...
    for (const missedCall of missedCalls) {
        ...
        processed += 1;
        // Small delay every 10 records to yield to event loop/other queries
        if (processed % 10 === 0) {
            await new Promise(r => setTimeout(r, 50));
        }
    }

    return { processed, totalFound: missedCalls.length };
}
