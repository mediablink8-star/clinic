...
async function triggerOutboundCall({ clinic, phone, missedCallId, patientName }) {
    ...
    if (!apiKey) {
        logger.error('VAPI_API_KEY not set in environment');
        return { success: false, reason: 'no_api_key' };
    }

    const backendUrl = process.env.BACKEND_API_URL || '';
    if (!backendUrl) {
        logger.warn('Vapi BACKEND_API_URL is not set — webhooks will fail.');
    }

    if (!clinic.vapiAssistantId && !process.env.VAPI_ASSISTANT_ID) {
        logger.warn('Vapi No assistant ID configured');
        return { success: false, reason: 'no_assistant' };
    }

    if (!clinic.vapiPhoneNumberId && !process.env.VAPI_PHONE_NUMBER_ID) {
        logger.warn('Vapi No phone number ID configured');
    ...
    try {
        ...
        const clinicTz = clinic.timezone || DEFAULT_TIMEZONE;
        const todaySlots = await _getSlots(clinic.id, today, clinicTz, 60, null);
        const tomorrowSlots = await _getSlots(clinic.id, tomorrow, clinicTz, 60, null);
        ...
        logger.warn('Vapi Data fetch failed', { error: err.message });
    }
    ...
    try {
        ...
        if ((result.status === 200 || result.status === 201) && result.data?.id) {
            logger.info('Outbound call triggered', { callId: result.data.id, phoneTail: phone.slice(-4), status: result.data.status });
            return { success: true, callId: result.data.id };
        }
        logger.warn('Vapi Outbound call failed', { response: result.data });
        return { success: false, reason: result.data?.message || JSON.stringify(result.data) };
    } catch (err) {
        logger.error('Vapi triggerOutboundCall error', { error: err.message });
...
async function getCallDetails(callId) {
    ...
    try {
        ...
        logger.error('Vapi getCallDetails error', { error: err.message });
        return null;
    }
}
