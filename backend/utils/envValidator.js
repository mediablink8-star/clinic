...
function validateEnv() {
    ...
    if (process.env.NODE_ENV === 'production') {
        required.push('SMTP_HOST', 'REDIS_URL');
    }
    ...
    // Remove duplicates from recommended if they were moved to required
    if (process.env.NODE_ENV !== 'production') {
        recommended.push('SMTP_HOST', 'REDIS_URL');
    }
    ...
    if (!process.env.REDIS_URL && process.env.NODE_ENV === 'production') {
        console.warn('⚠️  REDIS_URL not set — background jobs will use single-process DB polling fallback. Provision Redis (e.g. Upstash) for cluster-safe scheduling.');
    }
