...
function getBookingLink(clinicId, missedCallId = null) {
  ...
  const baseUrl = (process.env.FRONTEND_URL || CONFIG_FRONTEND_URL || '').replace(/\/+$/, '');
if (!baseUrl) throw new Error('FRONTEND_URL is required — booking links cannot be generated');
