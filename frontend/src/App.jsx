...
const App = () => {
  ...
  const refetchApts = useCallback(async () => {
    if (isFetchingAptsRef.current) return;
    ...
    try {
      const res = await api.get(`/appointments?_t=${Date.now()}`);
  ...
  useEffect(() => {
    if (!token) return;
    aptIntervalRef.current = setInterval(() => refetchApts(), 15000);
    return () => { if (aptIntervalRef.current) clearInterval(aptIntervalRef.current); };
  }, [token, refetchApts]);
...
const handleLogin = (loginData) => {
     const { token, clinic } = loginData;
      ...
      const tabs = ['dashboard', 'calendar', 'appointments', 'patients', 'reports', 'analytics', 'settings', 'ai', 'admin'];
...
{showOnboarding && (
      ...
      {showShortcuts && (
        <div onClick={() => setShowShortcuts(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
