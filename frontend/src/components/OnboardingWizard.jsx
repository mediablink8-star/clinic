...
{current.key === 'voice' && (
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '2rem', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        {skippable && !isLast && (
                            <button onClick={handleSkip} style={{ background: 'none', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.5)', padding: '8px 14px', borderRadius: '12px', cursor: 'pointer', fontSize: '0.82rem', fontWeight: '700' }}>
                                Παράλειψη →
                            </button>
                        )}
                        {isLast ? (
                            <button onClick={handleFinish} disabled={saving} style={{ padding: '11px 24px', borderRadius: '14px', border: 'none', background: '#10b981', color: 'white', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 8px 20px -4px rgba(16,185,129,0.4)' }}>
                                {saving ? <><Loader size={15} style={{ animation: 'spin 1s linear infinite' }} /> Ολοκλήρωση...</> : <>Πάμε στο Dashboard <ArrowRight size={16} /></>}
                            </button>
                        ) : (
