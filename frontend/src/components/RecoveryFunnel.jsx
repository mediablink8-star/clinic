...
const RecoveryFunnel = ({ logs = [], stats = {} }) => {
    ...
    const smsSent   = logArray.filter(l => l && (l.smsStatus === 'sent' || l.smsStatus === 'simulated')).length || 0;
    const responded = logArray.filter(l => l && ['RECOVERING', 'RECOVERED'].includes(l.status) && (l.smsStatus === 'sent' || l.smsStatus === 'simulated')).length || 0;
