...
const SendMessageModal = ({ 
    ...
    const filtered = patients.filter(p =>
        (p.patientName || p.name)?.toLowerCase().includes(search.toLowerCase()) || p.phone?.includes(search)
    );
    ...
    const handleSend = async () => {
        ...
        try {
            const resp = await api.post('/messages/send', {
                patientId: selected.patientId || selected.id,
                            ...
                            {filtered.length === 0 ? (
                                ...
                                >
                                    <div style={{ 
                                        ...
                                        {(p.patientName || p.name || '?')[0]}
                                    </div>
                                    <div>
                                    <p style={{ fontWeight: '700', fontSize: '0.875rem', margin: 0 }}>{p.patientName || p.name || 'Άγνωστος'}</p>
                        ...
                        <div style={{ 
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <div style={{ 
                                    ...
                                    {(selected.patientName || selected.name || '?')[0]}
                                </div>
                                <div>
                                    <p style={{ fontWeight: '700', fontSize: '0.875rem', margin: 0 }}>{selected.patientName || selected.name || 'Άγνωστος'}</p>
