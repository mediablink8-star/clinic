...
const MiniSparkline = ({ data = [], color = '#6366f1', width = 80, height = 28 }) => {
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ flexShrink: 0 }}>
      ...
      <polygon points={`0,${height} ${pts} ${width},${height}`} fill={`${color}0A`} />
    </svg>
  );
};

const StatCard = ({ icon: Icon, color, bg, title, value, subtitle, trendValue, trendType, onClick, delay, highlighted = false, size, sparklineData }) => {
    const isCompact = size === 'compact';
    const iconBackground = bg || `${color}14`;

    return (
        <div
            className={`card-glass stat-card shine-on-hover animate-fade group ${highlighted ? 'stat-card--highlighted' : ''}`}
            style={{
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    {sparklineData && !isCompact && <MiniSparkline data={sparklineData} color={color} />}
                    {(trendValue) && (
