...
const AISettings = ({ clinic, token, onUpdate }) => {
    ...
    const handleTestGemini = async () => {
        ...
        try {
            ...
            if (res.data.success) showToast(`Gemini συνδέθηκε! Απάντηση: ${res.data.response}`);
            else showToast(`Σφάλμα Gemini: ${res.data.error}`, 'error');
        } catch (err) {
            showToast('Αποτυχία δοκιμής Gemini', 'error');
        } finally {
