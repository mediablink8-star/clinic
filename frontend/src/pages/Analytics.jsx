...
const Analytics = ({ recoveryLog = [], recoveryStats = {}, spending = {}, systemStats = {}, doctorAnalytics = [], loading }) => {
    if (loading) {
    ...
    const responded = logs.filter(l => ['RECOVERING', 'RECOVERED'].includes(l.status) && (l.smsStatus === 'sent' || l.smsStatus === 'simulated')).length;
    const booked = recoveryStats.recovered || 0;
    const recoveryRate = totalMissed > 0 ? Math.max(0, Math.round((booked / totalMissed) * 100)) : 0;
    const smsDeliveryRate = smsSent > 0 ? Math.max(0, Math.round(((smsSent - smsFailed) / smsSent) * 100)) : 0;
    const responseRate = smsSent > 0 ? Math.min(100, Math.max(0, Math.round((responded / smsSent) * 100))) : 0;
                            ...
                            <div key={label} style={{ background: 'var(--card-bg)', backdropFilter: 'blur(10px)', borderRadius: '16px', border: '1px solid var(--border)', padding: '1.1rem 1.25rem', boxShadow: 'var(--shadow-sm)' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '3px', padding: '2px 7px', borderRadius: '99px', background: neutral ? 'var(--bg-subtle)' : up ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: neutral ? 'var(--text-light)' : up ? '#10b981' : '#ef4444', fontSize: '0.72rem', fontWeight: '800' }}>
                                        {neutral ? '—' : (up ? '+' : '') + d + (isRate ? '%' : '') + ' vs παρ. εβδομάδα'}
                                    </div>
                                </div>
