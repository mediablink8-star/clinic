...
import { Building2, Mail, Lock, Phone, ArrowRight, CheckCircle2, Key } from 'lucide-react';
      ...
      <div className="register-right">
        <div style={{
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {/* Invite Code */}
            <div>
              <label htmlFor="register-invite-code" style={{ fontSize: '0.75rem', fontWeight: '700', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: '6px' }}>Κωδικός Πρόσκλησης (Προαιρετικό)</label>
              <div style={{ position: 'relative' }}>
                <Key size={16} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.3)' }} />
                <input
                  id="register-invite-code"
                  type="text"
                  value={formData.inviteCode}
                  onChange={e => setFormData(prev => ({ ...prev, inviteCode: e.target.value }))}
                  placeholder="Εισάγετε τον κωδικό πρόσκλησης αν απαιτείται"
                  style={{ width: '100%', padding: '13px 14px 13px 42px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.07)', color: 'white', fontSize: '0.9rem', outline: '2px solid transparent', boxSizing: 'border-box' }}
                />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', margin: '0.5rem 0' }}>
              ...
              <label htmlFor="agreedToTerms" style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.4, cursor: 'pointer' }}>
                Αποδέχομαι τους <a href="/dpa" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)', fontWeight: '600', textDecoration: 'underline' }}>Όρους Χρήσης / DPA</a> και την <a href="/privacy" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)', fontWeight: '600', textDecoration: 'underline' }}>Πολιτική Απορρήτου (GDPR)</a> του ClinicFlow.
              </label>
            </div>
