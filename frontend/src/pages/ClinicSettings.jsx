...
const SECTIONS = [
    ...
    { id: 's-holidays', number: '9', label: 'Αργίες', icon: <Calendar size={14} color="var(--primary)" />, iconBg: 'var(--primary-light)', title: 'Αργίες & Ημέρες Αργίας', subtitle: 'Ελληνικές εθνικές αργίες και κλειστές ημέρες' },
    { id: 's-danger', number: '10', label: 'Επαναφορά', icon: <Trash2 size={14} color="var(--urgent)" />, iconBg: 'var(--error-light)', title: 'Επικίνδυνη Ζώνη', subtitle: 'Επαναφορά και διαγραφή' },
];
...
const ClinicSettings = ({ clinic, token, onUpdate }) => {
    ...
    const handleUpgrade = async (planKey) => {
        ...
        showToast('Για αναβάθμιση πακέτου επικοινωνήστε με την υποστήριξη: support@clinicflow.app', 'info', 8000);
    };
    ...
    const handleDeactivateDoctor = async (doctorId) => {
        const ok = await confirm('Είστε σίγουροι ότι θέλετε να απενεργοποιήσετε αυτόν τον γιατρό;', { title: 'Απενεργοποίηση Γιατρού', variant: 'warning' });
        if (!ok) return;
        try {
    ...
    const handleRemoveMember = async (id, email) => {
        const ok = await confirm(`Αφαίρεση του μέλους ${email} από την ομάδα;`, { title: 'Αφαίρεση Μέλους', variant: 'danger' });
        if (!ok) return;
        try {
    ...
    const handleDisableMfa = async () => {
        const ok = await confirm('Είστε σίγουροι ότι θέλετε να απενεργοποιήσετε το MFA; Αυτό μειώνει την ασφάλεια του λογαριασμού σας.', { title: 'Απενεργοποίηση MFA', variant: 'warning' });
        if (!ok) return;
    ...
    const [webhookData, setWebhookData] = React.useState({
        ...
        webhookDirectSms: clinic?.webhookDirectSms || '',
        webhookInboundSms: clinic?.webhookInboundSms || '',
        webhookReviewRequest: clinic?.webhookReviewRequest || '',
    });
    const [testingAll, setTestingAll] = React.useState(false);
    const [testAllResult, setTestAllResult] = React.useState(null);
    ...
    const handleTestWebhook = async (field, url) => {
        if (!url) {
            showToast('Εισάγετε πρώτα URL webhook.', 'error');
            return;
        }
        ...
        try {
            ...
            setTestingWebhooks(prev => ({
                ...prev,
                [field]: success ? 'connected' : 'failed',
                [`${field}_latency`]: res.data?.latency,
                [`${field}_error`]: res.data?.error,
            }));
            if (success) {
                showToast(`Webhook ${field} OK (${res.data.latency || '?'}ms)`, 'success');
            } else {
                const errMsg = res.data?.error || 'Άγνωστο σφάλμα';
                showToast(`Αποτυχία webhook: ${errMsg}`, 'error');
            }
        } catch (err) {
            const status = err.response?.status;
            const backendMsg = err.response?.data?.error || err.response?.data?.message;
            let msg;
            if (status === 400) msg = backendMsg || 'Μη έγκυρο URL.';
            else if (status === 403) msg = backendMsg || 'Δεν επιτρέπονται private/local URLs.';
            else if (!err.response) msg = 'Αδυναμία σύνδεσης με τον διακομιστή.';
            else msg = backendMsg || `Σφάλμα (${status || 'unknown'})`;
            setTestingWebhooks(prev => ({ ...prev, [field]: 'failed', [`${field}_error`]: msg }));
            showToast(msg, 'error');
        }
    };
    ...
    const handleResetDefaults = async () => {
        const ok = await confirm('Αυτή η ενέργεια θα επαναφέρει όλες τις ρυθμίσεις AI, τις υπηρεσίες και το ωράριο στις αρχικές προεπιλογές. Δεν μπορεί να αναιρεθεί.', { title: 'Επαναφορά Προεπιλογών', confirmLabel: 'Επαναφορά', variant: 'danger' });
        if (!ok) return;
    ...
    React.useEffect(() => {
        ...
        if (holidays.length === 0 && !holidaysLoading) {
            setHolidaysLoading(true);
            api.get('/admin/holidays?months=12')
    ...
    const handleDisconnectGoogleCalendar = async () => {
        const ok = await confirm('Αποσύνδεση Google Calendar; Τα συγχρονισμένα ραντεβού δεν θα διαγραφούν.', { title: 'Αποσύνδεση Google Calendar', confirmLabel: 'Αποσύνδεση', variant: 'warning' });
        if (!ok) return;
        try {
...
<button type="button" className="btn btn-primary" onClick={handleSaveVapi} disabled={savingVapi}>
            ...
            <SectionCard id="s7" number="6" icon={<Zap size={15} color="#f59e0b" />} iconBg="#fffbeb"
                ...
                {[
                    { key: 'webhookMissedCall',      label: 'Workflow 1 — Missed Call Recovery',         hint: 'https://n8n-znz6.srv1664589.hstgr.cloud/webhook/missed-call' },
                    { key: 'webhookAppointment',     label: 'Workflow 2 — Appointment Confirmation',      hint: 'https://n8n-znz6.srv1664589.hstgr.cloud/webhook/appointment-confirmation' },
                    { key: 'webhookReminders',       label: 'Workflow 3 — Reminder (24h before)',         hint: 'https://n8n-znz6.srv1664589.hstgr.cloud/webhook/appointment-reminder' },
                    { key: 'webhookDirectSms',       label: 'Workflow 4 — Manual SMS Send',               hint: 'https://n8n-znz6.srv1664589.hstgr.cloud/webhook/send-sms' },
                    { key: 'webhookInboundSms',      label: 'Workflow 5 — Inbound SMS Reply',             hint: 'https://n8n-znz6.srv1664589.hstgr.cloud/webhook/inbound-sms' },
                    { key: 'webhookReviewRequest',   label: 'Workflow 6 — Review Request',                hint: 'https://n8n-znz6.srv1664589.hstgr.cloud/webhook/review-request' },
                    { key: 'webhookUrl',             label: 'Global Fallback Webhook URL',                hint: 'Χρησιμοποιείται αν δεν οριστεί ειδικό webhook (π.χ. https://n8n-znz6.srv1664589.hstgr.cloud/webhook)' },
                ].map(({ key, label, hint }) => (
                ...
                <div style={{
                    ...
                    background: 'linear-gradient(135deg, rgba(99,102,241,0.05), rgba(13,148,136,0.05))',
                    border: '1px solid rgba(99,102,241,0.15)',
                }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                        <div>
                            <h4 style={{ fontSize: '0.92rem', fontWeight: '800', color: 'var(--secondary)', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <Zap size={16} /> Δοκιμή και των 6 webhooks μονομιάς
                            </h4>
                            <p style={{ fontSize: '0.78rem', color: '#64748b', margin: '4px 0 0 0' }}>
                                Στέλνει ένα test event σε κάθε ένα από τα 6 n8n workflows παράλληλα. Χρησιμοποιήστε το στο onboarding checklist.
                            </p>
                        </div>
                        <button
                            type="button"
                            ...
                            onClick={async () => {
                                ...
                                try {
                                    const res = await api.post('/clinic/webhooks/test-all');
                                    setTestAllResult(res.data);
                                    const s = res.data?.summary;
                                    if (s?.failed === 0 && s?.skipped === 0) {
                                        showToast(`✅ Και τα ${s.total} webhooks απάντησαν επιτυχώς`, 'success');
                                    } else if (s?.skipped > 0) {
                                        showToast(`⚠️ ${s.skipped} είναι χωρίς URL — συμπληρώστε τα πρώτα`, 'warning');
                                    } else {
                                        showToast(`❌ ${s.failed} από ${s.total} απέτυχαν`, 'error');
                                    }
                                } catch (err) {
                                    const backendMsg = err.response?.data?.error || err.response?.data?.message;
                                    showToast(backendMsg || 'Αποτυχία δοκιμής webhooks', 'error');
                                } finally {
                                    setTestingAll(false);
                                }
                            }}
                        >
                            {testingAll ? <><Loader size={14} className="animate-spin" /> Δοκιμή...</> : <><Zap size={14} /> Test all 6</>}
                        </button>
                    </div>
                    {testAllResult?.results && (
                        <div style={{ marginTop: '1rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '8px' }}>
                            {testAllResult.results.map((r) => {
                                const isOk = r.success;
                                const isSkip = r.skipped;
                                const bg = isSkip ? 'rgba(148,163,184,0.08)' : isOk ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)';
                                const border = isSkip ? 'rgba(148,163,184,0.2)' : isOk ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)';
                                const label = isSkip ? '⊘' : isOk ? '✓' : '✗';
                                const color = isSkip ? '#64748b' : isOk ? '#10b981' : '#ef4444';
                                return (
                                    <div key={r.eventType} style={{ padding: '10px 12px', background: bg, border: `1px solid ${border}`, borderRadius: '10px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                                            <span style={{ color, fontWeight: '900' }}>{label}</span>
                                            <span style={{ fontSize: '0.78rem', fontWeight: '700', color: 'var(--text)' }}>{r.label}</span>
                                        </div>
                                        <div style={{ fontSize: '0.7rem', color: '#64748b' }}>
                                            {isSkip ? r.error : isOk ? `${r.attempts} προσπάθεια, ${r.duration}ms` : r.error || 'Αποτυχία'}
                                        </div>
                                    </div>
                                );
                            })}
            ...
            {/* 9 · Holidays */}
            <SectionCard id="s-holidays" number="9" icon={<Calendar size={15} color="var(--primary)" />} iconBg="var(--primary-light)"
                title="Αργίες & Ημέρες Αργίας" subtitle="Ελληνικές εθνικές αργίες για τους επόμενους 12 μήνες">
                {holidaysLoading ? <p style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--text-light)' }}>Φόρτωση αργιών…</p> : holidays.length === 0 ? (
                    <p style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--text-light)' }}>Δεν υπάρχουν αργίες. Εκτελέστε <code>npm run seed:holidays</code> για να φορτώσετε τις ελληνικές εθνικές αργίες.</p>
                ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '10px' }}>
                        {holidays.map(h => {
                            const d = new Date(h.date);
                            const day = d.getUTCDate();
                            const month = d.toLocaleDateString('el-GR', { month: 'long', timeZone: 'UTC' });
                            const weekday = d.toLocaleDateString('el-GR', { weekday: 'long', timeZone: 'UTC' });
                            return (
                                <div key={h.id} style={{
                                    ...
                                    <div style={{
                                        ...
                                        minWidth: '48px', padding: '6px 0', borderRadius: '10px',
                                        background: 'var(--primary-light)', color: 'var(--primary)'
                                    }}>
                                        <span style={{ fontSize: '1.1rem', fontWeight: '900', lineHeight: 1 }}>{day}</span>
                                        <span style={{ fontSize: '0.65rem', fontWeight: '700', textTransform: 'uppercase', marginTop: '2px' }}>{month}</span>
                                    </div>
                                    <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
                                        <span style={{ fontSize: '0.85rem', fontWeight: '700', color: 'var(--text)' }}>{h.name}</span>
                                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{weekday}</span>
                                    </div>
                                </div>
                            );
                        })}
                ...
                <p style={{ marginTop: '1rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    Κατά τη διάρκεια αργιών τα μηνύματα SMS αποστέλλονται την επόμενη εργάσιμη ημέρα στις 09:00.
                </p>
            </SectionCard>

            {/* 10 · Danger Zone */}
            <SectionCard id="s-danger" number="10" icon={<Trash2 size={15} color="var(--urgent)" />} iconBg="var(--error-light)"
